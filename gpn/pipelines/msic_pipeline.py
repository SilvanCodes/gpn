...
class MSICPipeline(ChunkPipeline):
    ...
    - msic (the MSIC score using p_ref and p_max at each position)
    """
    ...
    def postprocess(self, all_model_outputs):
        ...
        # Vectorized p_ref and MSIC computation
        ref_idx = pd.Categorical(df["ref"], categories=acgt).codes
        df["p_ref"] = probs[np.arange(len(df)), ref_idx]

        ic = 1 + np.sum(probs * (np.log(probs) / np.log(4)), axis=1)
        p_max = np.max(probs, axis=1)
        p_ref = df["p_ref"].to_numpy()
        df["msic"] = (1 - 2 * (p_max - p_ref) / p_max) * ic

        cols = ["ref", "p_ref"] + [f"p_{n}" for n in acgt] + ["msic"]
        return df[cols]
